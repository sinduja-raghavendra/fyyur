...
def shows():
  ...
  # TODO: replace with real venues data.
  #       num_shows should be aggregated based on number of upcoming shows per venue.
  shows=Show.query.join(Venue).join(Artist).add_columns(Show.id, Venue.id, Artist.id, Artist.name, Artist.image_link, Show.start_time, Venue.name).all()
  data = []
  for show in shows:
    showList = {}
    showList['venue_id']= show[2]
    showList['venue_name']= show.name
    showList['artist_id']=  show[3]
    showList['artist_name']= show[4]
    showList['artist_image_link']= show[5]
